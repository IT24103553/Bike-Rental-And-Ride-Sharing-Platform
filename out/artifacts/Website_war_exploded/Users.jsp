...
<head>
  ...
  <style>
    ...
    .btn-primary { background-color: #0d6eefd; }
...
<div class="sidebar">
  ...
  <a href="BikeListServlet"><i class="fas fa-motorcycle mr-2"></i> Bikes</a>
  ...
  <%
    ...
    try (BufferedReader userReader = new BufferedReader(new FileReader(USER_FILE_PATH))) {
      ...
      while ((line = userReader.readLine()) != null) {
        ...
        if (parts.length == 5) {
          usersList.add(new String[]{parts[0].trim(), parts[1].trim(), parts[3].trim(), parts[4].trim()});
        }
      }
    } catch (Exception e) {
  %>
  ...
  <div class="overflow-x-auto">
    <table class="custom-table w-full">
      ...
      <tr>
        <td><%= user[0] %></td>
        <td><%= user[1] %></td>
        <td><%= user[2] != null && !user[2].isEmpty() ? user[2] : "N/A" %></td>
        <td><%= user[3] %></td>
        <td>
          <a href="UpdateUser.jsp?username=<%= URLEncoder.encode(user[0], "UTF-8") %>" class="btn btn-primary btn-custom text-white mr-2">Update</a>
